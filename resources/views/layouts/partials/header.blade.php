                        ...
                        <div class="dropdown-menu content-dd dropdown-menu-end dropdown-menu-animate-up"
                            ...
                            <div class="message-body" data-simplebar>
                                @if (!$henkatens->isEmpty())
                                    @foreach ($henkatens as $henkaten)
                                        @php
                                            if ($henkaten->status == 'stop') {
                                                $color = 'danger';
                                                $shape = 'x';
                                            } else {
                                                $color = 'warning';
                                        ...
                                        <div class="d-flex align-items-center justify-content-between">
                                            <a href="{{ url('/dashboard/' . $henkaten->line->id) }}"
                                                ...
                                                <div class="w-75 d-inline-block v-middle">
                                                    <h6 class="mb-1 fw-semibold">{{ $henkaten->abnormality }}</h6>
                                                    <span class="d-block">{{ $henkaten->line->name }}</span>
                                                </div>
                                                <span class="badge bg-{{ $colorStatus }} rounded-4 py-1 lh-sm"
                                                    style="font-size: 10px">{{ $text }}</span>
                                            </a>
                                        </div>
                                    @endforeach
                                @else
                                    <p class="text-dark w-50 text-center mt-2" style="margin-left: 80px">No
                                        Abnormality :)</p>
                                @endif
                            </div>
                            ...
                            <div class="profile-dropdown position-relative" data-simplebar>
                                ...
                                <div class="d-flex align-items-center py-9 mx-7 border-bottom">
                                    ...
                                    <div class="ms-3">
                                        ...
                                        <span
                                            class="mb-2 mt-2 d-block badge bg-light-info text-info">{{ auth()->user()->role }}</span>
